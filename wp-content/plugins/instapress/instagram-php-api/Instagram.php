...
class Instagram {
    ...
    */
     ...
     */
    public function getUserFeed($maxId = null, $count = 0, $minId = null) {
        $this->_init();
        $endpointUrl = sprintf($this->_endpointUrls['user_feed'], $this->_accessToken, $maxId, $minId, $count);
        $this->_initHttpClient($endpointUrl);
        return $this->_getHttpClientResponse();
    }
     ...
     */
    public function getUserRecent($id, $maxId = '', $count = 0, $minId = '', $maxTimestamp = '', $minTimestamp = '') {
        $this->_init();
        if($maxId == 0) // 2011-10-18: Instagrams API no longer seems to allow 0
        	$maxId = "";
        $endpointUrl = sprintf($this->_endpointUrls['user_recent'], $id, $this->_accessToken, $maxId, $minId, $maxTimestamp, $minTimestamp, $count);
        $this->_initHttpClient($endpointUrl);
        return $this->_getHttpClientResponse();
    }
